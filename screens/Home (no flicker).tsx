...
import { View, TouchableOpacity, Text, Animated, TouchableWithoutFeedback, Image, ActivityIndicator, ScrollView, Modal, FlatList, TextInput, Alert } from 'react-native';
...
const Home: React.FC = () => {
  ...
  const handleGroupPurchaseStart = async (voucher: Voucher) => {
    try {
      ...
      Alert.alert(
        ...
        `🎉 Your group purchase has been created successfully!\n✨ You earned ${xpResult.xpGained} XP!`,
        [
          {
            text: 'Awesome!',
            onPress: () => {
              if (xpResult.leveledUp) {
                Alert.alert(
                  ...
                  [
                    {
                      text: 'Great!',
                      onPress: () => navigation.navigate('GroupPurchase', { groupPurchaseId: response.id })
                    }
                  ]
                );
              } else {
  ...
  const handleActivateXpDoubler = () => {
    ...
    Alert.alert(
      ...
      '🎉 All XP rewards will be doubled for the next 15 minutes! 🌟\nCheck the ⌛ icon to see remaining time.',
      [{ text: 'Awesome!' }]
    );
    setTimerMessage(false); // Remove the old message display
  };
